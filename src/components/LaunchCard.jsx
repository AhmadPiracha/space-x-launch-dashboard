...
export default function LaunchCard({ launch, rocketName, onClick }) {
  return (
    <div
      className="overflow-hidden hover:border-accent transition-all cursor-pointer group hover:shadow-lg hover:shadow-accent/20 border border-border/50 backdrop-blur-sm rounded-lg bg-card p-5"
      onClick={onClick}
    >
      ...
      <button
        className="w-full bg-background/50 border border-accent/30 hover:border-accent hover:bg-accent/10 text-accent group-hover:text-accent transition-all gap-2 p-2 rounded-md text-sm font-medium flex items-center justify-center"
        onClick={(e) => {
          e.stopPropagation()
        }}
      >
